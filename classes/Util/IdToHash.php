...
class IdToHash {
	...
	public function encode($number, $minLen=0, $padSeed=0){
		/*
		    def encode_url(self, n, min_length=MIN_LENGTH):
		        return self.enbase(self.encode(n), min_length)
		 */
		return $this->toBase($this->encodeNumber($number), $minLen, $padSeed);
	}
	...
	protected function toBase($number, $minLen, $padSeed=0){
		/*
		...
		for ($i = 0; $i < $paddingSize; $i++){
			$padding .= $nullAlphabet[ ($i + $number + $padSeed) % $nullAlphabetSize];
		}
